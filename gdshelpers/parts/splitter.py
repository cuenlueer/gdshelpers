...
class Splitter:
    def __init__(
        ...
        if implement_cadence_bug:
            ...
            warnings.warn(
                "The usage of implement_cadence_bug is deprecated. It will be removed in the next version.",
                DeprecationWarning,
            )
    ...
    @classmethod
    def make_at_left_branch_port(
        cls, port, total_length, sep, wavelength_root=None, **kwargs
    ):
        wavelength_root = wavelength_root or port.width

        origin = shapely.geometry.Point(-sep / 2, total_length)
        origin = shapely.affinity.rotate(
            origin, -np.pi / 2 + port.angle, origin=[0, 0], use_radians=True
        )
    ...
    def _generate(self):
        ...
        if self._implement_cadence_bug:
            ...
            polygon2 = shapely.affinity.scale(
                polygon1, xfact=-1, origin=[0, 0, 0]
            )
            ...
            polygon = shapely.affinity.translate(
                polygon, self._origin[0], self._origin[1]
            )
            ...
            port_points = shapely.affinity.rotate(
                port_points,
                -np.pi / 2 + self._angle,
                origin=[0, 0],
                use_radians=True,
            )
            port_points = shapely.affinity.translate(
                port_points, self._origin[0], self._origin[1]
            )

            self._polygon = polygon
            self._ports["root"] = Port(
                port_points[0].coords[0], self._angle + np.pi, width=self._wl
            )
            self._ports["left_branch"] = Port(
                port_points[1].coords[0], self._angle, width=self._wr
            )
            self._ports["right_branch"] = Port(
                port_points[2].coords[0], self._angle, width=self._wr
            )
            ...
            half_final_width = (self._wl + self._wr) / 2.0
    ...
    def _connect_two_points(p1, p2, n_points=20):
        ...
        upper_half = [
            np.cos(phi) * r - ht + r + p1[0],
            np.sin(phi) * r + tl + p1[1],
        ]
...
class DirectionalCoupler:
    ...
    @classmethod
    def make_at_port(
        cls, port, length, gap, bend_radius, bend_angle=np.pi / 5.0, which=0
    ):
        """
        ...
        if which == 0:
            ...
            offset = shapely.geometry.Point(
                0,
                -4 * (1 - np.cos(bend_angle)) * bend_radius - gap - port.width,
            )
            offset = shapely.affinity.rotate(
                offset, port.angle, origin=[0, 0], use_radians=True
            )
            return cls(
                port.origin + offset,
                port.angle,
    ...
    def _generate(self):
        ...
        self.left_ports.append(
            self.left_ports[0].parallel_offset(
                4
                * (
                    self._bend_radius
...
class MMI:
    ...
    def make_at_port(
        ...
        :return:
        """
        input_positions, output_positions = cls._calculate_positions(
            num_inputs, num_outputs
        )
        positions = input_positions if pos[0] == "i" else output_positions
        offset = shapely.geometry.Point(
            taper_length + (0 if pos[0] == "i" else length),
            width * positions[int(pos[1])],
        )
        offset = shapely.affinity.rotate(
            offset, port.angle, origin=[0, 0], use_radians=True
        )
        ...
        return cls(
            [port.origin[i] + offset.coords[0][i] for i in range(2)],
            port.angle + (np.pi if pos[0] == "o" else 0),
            port.width,
            length,
            width,
            num_inputs,
    ...
    def _calculate_positions(cls, num_inputs, num_outputs):
        if num_inputs > 2 or num_outputs > 2:
            ...
            raise NotImplementedError(
                "Positions may not be correct -> checking needed"
            )

        if num_inputs == 2 and num_outputs == 2:
            input_positions = [-1.0 / 6, 1.0 / 6]
            output_positions = [-1.0 / 6, 1.0 / 6]
            ...
            input_positions = (
                ...
                / num_inputs
                * np.linspace(
                    0.5 - 0.5 * num_inputs, -0.5 + 0.5 * num_inputs, num_inputs
                )
            )
            output_positions = (
                1.0
    ...
    def _generate(self):
        ...
        for pos in input_positions:
            ...
            wg.add_straight_segment(
                self._taper_length, final_width=self._wg_width
            )
            self._wgs.append(wg)
