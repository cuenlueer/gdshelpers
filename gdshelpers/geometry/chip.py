...
from gdshelpers.geometry.shapely_adapter import (
    convert_to_layout_objs,
    bounds_union,
    transform_bounds,
)
...
# Top-level processing function (pickleable)
def _process_layer_worker(args):
    layer_spec, chain = args
    layer_data = []
    for polygon in chain:
...
class Cell:
        ...
        if layers is None and self._bounds is not None:
            ...
            for layer in layers or self.layer_dict.keys():
                for geo in self.layer_dict.get(layer, []):
                    geo_bounds = (
                        geo.get_shapely_object().bounds
                        if hasattr(geo, "get_shapely_object")
                        else geo.bounds
                    )
                    if (
                        geo_bounds != ()
                    ):  # Some shapely geometries (collections) can return empty bounds
                        bounds.append(geo_bounds)

            # Cache envelope if we have the global envelope
            if layers is None:
                self._bounds = (
                    bounds_union(bounds) if len(bounds) > 0 else None
                )
    ...
    def add_cell(
        ...
        """
        if cell.get_dlw_data() and cell.name in [
            cell_dict["cell"].name for cell_dict in self.cells
        ]:
            raise ValueError(
    ...
    def add_region_layer(
        self,
        region_layer: int = std_layers.regionlayer,
    ...
    def add_frame(
        ...
        padding=30.0,
        line_width=1.0,
        frame_layer: int = std_layers.framelayer,
        bounds=None,
    ):
        """
    ...
    def add_ebl_frame(
        self, layer: int, frame_generator, bounds=None, **kwargs
    ):
        """
        ...
        frame_generator = (
            frame_generator
            if callable(frame_generator)
            else getattr(ebl_frame_generators, frame_generator)
        )
        bounds = bounds or self.bounds
            ...
            for dlw_type, dlw_type_data in cell.get_dlw_data().items():
                for dlw_id, data in dlw_type_data.items():
                    ...
                    if sub_cell["angle"] is not None:
                        c, s = np.cos(sub_cell["angle"]), np.sin(
                            sub_cell["angle"]
                        )
                        data["origin"] = np.array([[c, -s], [s, c]]).dot(
                            data["origin"]
                        )
                        data["angle"] += sub_cell["angle"]
                    data["origin"] = (
                        np.array(origin) + data["origin"]
                    ).tolist()
    ...
    def get_desc(self):
        ...
        desc["cells"] = {
            cell["cell"].name: dict(
                offset=tuple(cell["origin"]),
                angle=cell["angle"] or 0,
                **cell["cell"].get_desc()
            )
            for cell in self.cells
        }
    ...
    def get_fractured_layer_dict(self, max_points=4000, max_line_points=4000):
        ...
        for layer, geometries in self.layer_dict.items():
            ...
            for geometry in geometries:
                geometry = (
                    geometry.get_shapely_object()
                    if hasattr(geometry, "get_shapely_object")
                    else geometry
                )
                ...
                geometry = itertools.chain(
                    *[
                        fracture_intelligently(
                            geo, max_points, max_line_points
                        )
                        for geo in geometry
                        if not geo.is_empty
                    ]
            ...
            fractured_layer_dict[layer] = itertools.chain(
                *fractured_geometries
            )
        ...
        if self.cell_gdspy is None:
            ...
            for sub_cell in self.cells:
                ...
                if (
                    sub_cell["columns"] == 1
                    and sub_cell["rows"] == 1
                    and not sub_cell["spacing"]
                ):
                    self.cell_gdspy.add(
                        gdspy.CellReference(
                            sub_cell["cell"].get_gdspy_cell(executor),
                            origin=sub_cell["origin"],
                            rotation=angle,
                            magnification=sub_cell["magnification"],
                    ...
                    self.cell_gdspy.add(
                        gdspy.CellArray(
                            ...
                            magnification=sub_cell["magnification"],
                            x_reflection=sub_cell["x_reflection"],
                            columns=sub_cell["columns"],
                            rows=sub_cell["rows"],
                            spacing=sub_cell["spacing"],
                        )
                    )
            for layer, geometries in self.layer_dict.items():
                for geometry in geometries:
                    if executor:
                        executor.submit(
                            ...
                            lambda future: self.cell_gdspy.add(future.result())
                        )
                    else:
                        self.cell_gdspy.add(
                            convert_to_layout_objs(
                                geometry, layer, library="gdspy"
                            )
                        )
    ...
    def get_oasis_cells(self, grid_steps_per_micron=1000, executor=None):
        ...
        if self.cell_oasis is None:
            ...
            for sub_cell in self.cells:
                x, y = sub_cell["origin"]
                x, y = round(x * grid_steps_per_micron), round(
                    y * grid_steps_per_micron
                )
                angle = (
                    np.rad2deg(sub_cell["angle"])
                    if sub_cell["angle"] is not None
                ...
                if not (
                    ...
                    repetition = fatamorgana.basic.GridRepetition(
                        [
                            round(
                                sub_cell["spacing"][0] * grid_steps_per_micron
                            ),
                            0,
                        ],
                        sub_cell["columns"],
                        [
                            0,
                            round(
                                sub_cell["spacing"][1] * grid_steps_per_micron
                            ),
                        ],
                        sub_cell["rows"],
                    )
                self.cell_oasis.placements.append(
                    fatamorgana.records.Placement(
                        False,
                        name=fatamorgana.NString(sub_cell["cell"].name),
                        x=x,
                        y=y,
                        angle=angle,
                ...
                for geometry in geometries:
                    if executor:
                        executor.submit(
                            ...
                            datatype=(
                                None if isinstance(layer, int) else layer[1]
                            ),
                            library="oasis",
                            ...
                            lambda future: self.cell_oasis.geometry.extend(
                                future.result()
                            )
                        )
                    else:
        ...
        return [self.cell_oasis] + [
            oasis_cell
            for sub_cell in self.cells
            for oasis_cell in sub_cell["cell"].get_oasis_cells(
                grid_steps_per_micron, executor
            )
        ]
    ...
    def save(
        ...
        if library == "gdshelpers":
            ...
            self.get_gdspy_lib().precision = (
                self.get_gdspy_lib().unit / grid_steps_per_micron
            )
            ...
            self.get_gdspy_lib().write_gds(
                name + ".gds", cells=[], binary_cells=binary_cells
            )
            ...
            with open(name + ".dxf", "w") as f:
                write_cell_to_dxf_file(
                    f, self, grid_steps_per_micron, parallel=parallel
                )
        else:
            raise ValueError(
        ...
        return geometric_union(
            ...
            + [
                translate_and_rotate(
                    cell["cell"].get_reduced_layer(layer),
                    cell["origin"],
                    cell["angle"],
                )
                for cell in self.cells
    ...
    def save_image(
        ...
        scale *= 5 / 127.0 if is_vector else 1.0
        ...
        fig.set_size_inches(
            np.asarray(
                (
                    actual_xlim[1] - actual_xlim[0],
                    actual_ylim[1] - actual_ylim[0],
                )
            )
            * scale
        )
        ...
        plt.savefig(
            filename, transparent=True, bbox_inches="tight", dpi=1 / resolution
        )
        plt.close()
    ...
    def add_dlw_marker(self, label: str, layer: int, origin, box_size=2.5):
        ...
        self.add_to_layer(
            std_layers.parnamelayer1,
            Text(origin, 2, label, alignment="center-center"),
        )

        self.add_dlw_data(
            "marker", label, {"origin": list(origin), "angle": 0}
        )

    def add_dlw_taper_at_port(
        self,
        label: str,
        layer: int,
        port: Port,
        ...
        self.add_to_layer(
            std_layers.parnamelayer1,
            Text(taper_port.origin, 2, label, alignment="center-center"),
        )
        ...
        self.add_dlw_data(
            ...
            str(label),
            {
                "origin": taper_port.origin.tolist(),
                "angle": port.angle,
                "starting_width": port.width,
                "taper_length": taper_length,
            },
        )
        if with_markers:
            for i, (v, l) in enumerate(
                itertools.product((-20, 20), (taper_length, 0))
            ):
                self.add_dlw_marker(
                    str(label) + "-" + str(i),
                    layer,
    ...
    def _gh_cell_to_kl_cell(self, layout):
        kl_cell = layout.create_cell(self.name)
        fractured_layers = self.get_fractured_layer_dict()

        # Process layers and polygons
        for layer, chain in fractured_layers.items():
            # Determine KLayout layer (cache if repeated)
            kl_layer = (
                layout.layer(layer, layer)
                if isinstance(layer, int)
                else layout.layer(*layer)
            )
            layer_shapes = kl_cell.shapes(kl_layer)

            # Convert all polygons in the chain to DPolygons
            for polygon in chain:
                coords = polygon.exterior.coords
                # Directly create DPoints from coordinate tuples
                dbpoints = [db.DPoint(x, y) for x, y in coords]
                layer_shapes.insert(db.DPolygon(dbpoints))

        # Process cell instances
        for cell_dict in self.cells:
            kl_child = cell_dict["cell"]._gh_cell_to_kl_cell(layout)

            # Precompute origin and spacing
            origin = cell_dict["origin"]
            angle_rad = cell_dict["angle"]
            angle_deg = angle_rad * 180 / np.pi if angle_rad else 0
            x_origin = origin[0] * 1000
            y_origin = origin[1] * 1000
            spacing = cell_dict["spacing"]
            x_disp = spacing[0] * 1000 if spacing else 0
            y_disp = spacing[1] * 1000 if spacing else 0

            # Create transformation
            trans = db.DCplxTrans(
                1.0,
                angle_deg,
                cell_dict["x_reflection"],
                x_origin,
                y_origin,
            )

            # Insert instance array
            kl_cell.insert(
                db.CellInstArray(
                    kl_child.cell_index(),
                    trans,
    ...
    def export_gds(self, filename: str = "output.gds"):
        if filename[-4:] != ".gds":
            filename = filename + ".gds"
        layout = db.Layout()
        self._gh_cell_to_kl_cell(layout)
...
if __name__ == "__main__":
    ...
    device_cell.add_dlw_taper_at_port(
        "A0", (1, 2), start_port.inverted_direction, 30
    )
