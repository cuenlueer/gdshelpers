...
def shapely_collection_to_basic_objs(collection):
    ...
    if isinstance(collection, (tuple, list)):
        ...
        objs = (
            hasattr(collection, "geoms")
            and collection.geoms
            or [
                collection,
            ]
        )
...
def cut_shapely_object(obj, x_axis=None, y_axis=None, other_side=False):
    ...
    if x_axis is not None:
        cut_boxes = [
            shapely.geometry.box(bbox[0][0], bbox[0][1], x_axis, bbox[1][1]),
            shapely.geometry.box(x_axis, bbox[0][1], bbox[1][0], bbox[1][1]),
        ]
    else:
        cut_boxes = [
            shapely.geometry.box(bbox[0][0], bbox[0][1], bbox[1][0], y_axis),
    ...
    try:
        ...
        for cut_box in cut_boxes:
            ...
            out_polygons.extend(
                hasattr(cut_polygon, "geoms")
                and list(cut_polygon.geoms)
                or [
                    cut_polygon,
                ]
            )
        ...
        warnings.warn(
            "Trying to cut an invalid polygon (possibly during gdsCAD conversion)."
        )
        warnings.warn(
            "We'll try to fix the polygon for you now, but try and figure out what's wrong, please."
        )
        return cut_shapely_object(
            obj.buffer(0), x_axis, y_axis, other_side=other_side
        )
...
def _number_of_points(poly):
    ...
    if hasattr(poly, "_gdsh_n_points"):
        ...
        poly_gdsh_n_points = sum(_number_of_points(p) for p in poly.geoms)

    elif type(poly) == shapely.geometry.Polygon:
        poly_gdsh_n_points = len(poly.exterior.coords) + sum(
            [
                len(shapely.geometry.LinearRing(x).coords)
                for x in list(poly.interiors)
            ]
        )
    else:
        poly_gdsh_n_points = len(poly.coords)

    return poly_gdsh_n_points
    ...
    while segments_joined:
        ...
        for i, polygon in enumerate(objs):
            ...
            for j, polygon2 in enumerate(objs):
                ...
                if max_points and polygon_points + _number_of_points(
                    polygon2
                ) >= (max_points - 2):
                    continue
                ...
                if polygon.touches(polygon2):
                    ...
                    if (
                        max_points
                        and _number_of_points(joined_poly) >= max_points
                    ):
                        continue
...
def fracture(obj, max_points_poly, max_points_line, max_interior=0):
    ...
    while done_counter < len(out_polygons):
        ...
        max_points = (
            max_points_poly
            if type(current_polygon) == shapely.geometry.Polygon
            else max_points_line
        )

        if (
            type(current_polygon) == shapely.geometry.Polygon
            and len(current_polygon.interiors) > max_interior
        ):
            uncut_polygon = out_polygons.pop(done_counter)
            cut_point = current_polygon.interiors[0].centroid.coords[0]
            shapes = cut_shapely_object(
                uncut_polygon, cut_point[0], cut_point[1]
            )
            extended_shapes = shapely_collection_to_basic_objs(shapes)
            out_polygons.extend(extended_shapes)

        elif (
            max_points and _number_of_points(current_polygon) > max_points > 0
        ):
            uncut_polygon = out_polygons.pop(done_counter)
...
def fracture_intelligently(
    obj, max_points, max_points_line, over_fracture_factor=1
):
    if over_fracture_factor >= 1:
        fractured_obj = fracture(
            obj, max_points / over_fracture_factor, max_points_line
        )
...
def geometric_union(objs):
    ...
    """
    objs = [
        obj.get_shapely_object() if hasattr(obj, "get_shapely_object") else obj
        for obj in objs
    ]
    return shapely.unary_union(objs)


def convert_to_gdscad(
    objs,
    layer=1,
    datatype=None,
    path_width=1.0,
    ...
    *max_points*/*over_fracture_factor* points and then healed again - which results in better fragmentation of some
    ...
    warnings.warn(
        "The usage of gdscad is deprecated as gdscad is not installable under python 3 from pypi."
        "Please switch to the built-in gds-export or gdspy.",
        DeprecationWarning,
    )

    return convert_to_layout_objs(
        objs,
        layer,
        datatype,
        path_width,
        path_pathtype,
        max_points,
        over_fracture_factor,
        max_points_line,
        library="gdscad",
    )


def convert_to_layout_objs(
    objs,
    layer=1,
    datatype=None,
    path_width=1.0,
    ...
    *max_points*/*over_fracture_factor* points and then healed again - which results in better fragmentation of some
    ...
    max_points = (
        ...
        else gdshelpers.configuration.point_limit
    )
    max_points_line = (
        max_points_line
        if max_points_line is not None
        else gdshelpers.configuration.point_limit_line
    )
    ...
    objs = (
        objs.get_shapely_object()
        if hasattr(objs, "get_shapely_object")
        else objs
    )
    ...
    for obj in fractured_objs:
        if type(obj) == shapely.geometry.Point:
            ...
            if path_width:
                if library == "gdscad":
                    ...
                    rounded_coords = np.multiply(
                        obj.coords, grid_steps_per_micron
                    ).astype(np.int)
                    exports_objs.append(
                        fatamorgana.records.Path(
                            (
                                rounded_coords[1:] - rounded_coords[:-1]
                            ).tolist(),
                            layer=layer,
                            datatype=datatype,
            ...
            assert (
                len(obj.interiors) <= 1
            ), "No polygons with more than one hole allowed, got %i" % len(
                obj.interiors
            )
            if library == "gdscad":
                exports_objs.append(
                    gdsCAD.core.Boundary(
                        obj.exterior.coords, layer=layer, datatype=datatype
                    )
                )
            elif library == "gdspy":
                exports_objs.append(
                ...
                rounded_coords = np.multiply(
                    obj.exterior.coords, grid_steps_per_micron
                ).astype(np.int)
                exports_objs.append(
...
def transform_bounds(bounds, origin, rotation=0, scale=1.0):
    """
